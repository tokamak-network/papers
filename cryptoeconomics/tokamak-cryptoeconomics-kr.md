...
*주의: 만약 백서 이외의 자료가 백서와 충돌하는 부분이 있다면 백서를 우선시하는 것으로 한다. 여기서 '백서'라 함은 이 문건, 즉  "토카막 레이어 2 암호경제(Tokamak layer 2 Cryptoeconomics)"를 의미한다.*
...
- **스테이킹(Staking)**: 토큰을 상응하는 컨트랙트로 이전해 네트워크의 보안에 기여하고 시뇨리지를 획득하는 행위이다. 이 노트에서는 레이어 1 상에서의 스테이킹만 다룬다.
